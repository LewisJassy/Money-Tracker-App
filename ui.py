...
def income():
    try:
        ...
        update_total_income_label(money_tracker.total_income)
...
def expense():
    try:
        ...
        if int(expense_entry.get()) > money_tracker.total_income:
            raise ValueError
...
income_entry.grid(row=1, column=4)

expense_entry = Entry(window, width=20)
expense_entry.grid(row=2, column=4)
...
add_income_button.grid(row=1, column=0, columnspan=2)

add_expense_button = Button(window, text="Add Expense", command=expense)
add_expense_button.grid(row=2, column=0, columnspan=2)

# A label to display the balance
balance_label = Label(window, text=f"Balance: ${money_tracker.balance}")
